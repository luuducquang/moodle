...
<xsl:template match="question">
    ...
    <thead>
        ...
        <xsl:text>&#x0a;</xsl:text>
        <tr>
            <td style="width: 1.0cm"><p class="{$colheading1_style}"><xsl:apply-templates select="name/text"/></p></td>
    ...
    <tbody>
        ...
        <tr>
        <td>
         <p style="font-weight: bold;" class="Cell" >Đáp án :</p>
        </td>
        <td colspan="3">
        <p style="font-weight: bold;width: 12.0cm" class="Cell"><xsl:apply-templates select="correct/text"/></p>
        </td>
        </tr>
...
<xsl:template match="answer[not(ancestor::subquestion)]|subquestion">
    ...
    <tr>
        ...
        <xsl:choose>
        <xsl:when test="$qtype = 'SA'">
            <td colspan="3" style="width: 2.0cm"><div class="chapter"><p class="Cell"><xsl:value-of select="normalize-space(text)"/></p></div></td>
        </xsl:when>
        <xsl:otherwise>
            <td colspan="3" style="width: 2.0cm"><div class="chapter"><xsl:apply-templates select="text|file"/></div></td>
        </xsl:otherwise>
        </xsl:choose>
